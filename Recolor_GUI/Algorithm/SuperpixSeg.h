...
public:
	...
	void SLIC() {
		...
		addWeighted(sobelImagex, 0.5, sobelImagey, 0.5, 0, sobelGradient);
	...
	void GetPixCntInEachSuperpix() {
		...
		cout << rgb_img.rows << " * " << rgb_img.cols << " = " << rgb_img.rows * rgb_img.cols << endl;
		int sum = 0;
		for (int i = 0; i < centers.size(); i++) {
			sum += centers[i].pix_cnt;
		}
		cout << "sum : " << sum << endl;
	}
	...
	void ShowDemo() {
		...
		//ShowRecoloredResult();
		ShowCenterAndBoundaryResult("D:\\Code\\Recoloring\\SuperPixelsResult\\man.png");
		cv::waitKey(0);
	}
};